...
export class StepExecutor {
  ...
  async executeStep(stepInfo) {
    ...
    const stagehand = await this.stagehandManager.getStagehandForWorkflow(
      workflow
    );
    ...
    try {
      ...
      console.log(
        `   🔄 执行 [${stepConfig.type}]: ${this.parser.describeStep(
          stepConfig
        )}`
      );
      ...
      console.log(`      步骤: ${executionRecord.stepConfig.parsedText}`);
  ...
  async executeObserve(config, stagehand) {
    ...
    if (elements.length > 0) {
      elements.forEach((element, index) => {
        const elementInfo =
          element.description || element.selector || "未知元素";
  ...
  async executeAgent(config, stagehand) {
    ...
    const agent = stagehand.agent({
      ...
      systemPrompt:
        "你是一个专业的网页自动化助手，能够准确执行用户指令并完成网页操作。",
    });
  ...
  async executeAct(config, stagehand) {
    ...
    // 添加额外的执行选项
    actParams.timeout = process.env.TEST_TIMEOUT
      ? parseInt(process.env.TEST_TIMEOUT)
      : 30000;
    actParams.retries = process.env.TEST_RETRIES
      ? parseInt(process.env.TEST_RETRIES)
      : 2;
  ...
  getStepStats() {
    const stats = {
      ...
      successfulExecutions: this.executionHistory.filter((h) => h.success)
        .length,
    ...
    // 计算平均耗时
    const successfulRecords = this.executionHistory.filter(
      (h) => h.success && h.duration
    );
    if (successfulRecords.length > 0) {
      const totalDuration = successfulRecords.reduce(
        (sum, record) => sum + record.duration,
        0
      );
      stats.averageDuration = Math.round(
        totalDuration / successfulRecords.length
      );
    }
  ...
  getRecentSteps(limit = 10) {
    return this.executionHistory.slice(-limit).map((record) => ({
