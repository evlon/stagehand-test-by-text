...
export class TextTestRunner {
  ...
  getStats() {
    ...
    return {
      ...
      totalSteps,
      passedSteps,
      successRate: totalCases > 0 ? (passedCases / totalCases * 100).toFixed(1) : 0
    };
  }
...
export function createTestSuite(textFilePath) {
  ...
  const suiteName = `文本测试: ${textFilePath.split('/').pop().replace('.txt', '').charAt(0).toUpperCase() + textFilePath.split('/').pop().replace('.txt', '').slice(1)}`;
        ...
        this.testCases.forEach((testCase, index) => {
          test(`TC${index + 1}: ${testCase.name}`, async () => {
            ...
            if (!result.passed) {
              ...
              throw new Error(`测试失败: ${failedStep?.error || '未知错误'}\n失败步骤: ${failedStep?.action}`);
            }
          }, 120000);
        });
      });
  ...
  switch (command) {
    case 'stats':
      ...
      if (Object.keys(stats).length === 0) {
        ...
        const totalActions = Object.values(stats).reduce((sum, s) => sum + s.cachedActions, 0);
